...
@lru_cache(maxsize=1)
def load_industry_rules() -> tuple[dict[str, str], list[dict[str, object]]]:
...
def normalize_industry(
    value: object,
    rules: tuple[dict[str, str], list[dict[str, object]]] | None = None,
) -> NormalizedIndustry:
    ...
    aliases, entries = rules or load_industry_rules()
    if text in aliases:

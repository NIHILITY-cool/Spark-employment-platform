...
function demoTrainingAnalysis(majorName, cityName) {
  ...
  const targetCategory = availableMajors[majorName] || '大数据开发'
  const skillMap = {
    大数据开发: [{ key: 'SQL', jobCount: 140 }, { key: 'Python', jobCount: 120 }, { key: 'Spark', jobCount: 88 }, { key: 'Java', jobCount: 65 }, { key: 'Hadoop', jobCount: 52 }],
    后端开发: [{ key: 'Java', jobCount: 160 }, { key: 'MySQL', jobCount: 118 }, { key: 'Redis', jobCount: 90 }, { key: 'Spring Boot', jobCount: 84 }, { key: 'Git', jobCount: 60 }],
    前端开发: [{ key: 'JavaScript', jobCount: 132 }, { key: 'Vue', jobCount: 108 }, { key: 'TypeScript', jobCount: 76 }, { key: 'CSS', jobCount: 68 }, { key: '工程化', jobCount: 45 }],
    数据分析: [{ key: 'SQL', jobCount: 128 }, { key: 'Python', jobCount: 94 }, { key: 'Excel', jobCount: 86 }, { key: '可视化', jobCount: 58 }, { key: '统计分析', jobCount: 54 }],
    人工智能: [{ key: 'Python', jobCount: 118 }, { key: 'PyTorch', jobCount: 70 }, { key: '机器学习', jobCount: 68 }, { key: '算法', jobCount: 62 }, { key: '数据处理', jobCount: 50 }],
  }
  const factor = cityName ? 0.42 : 1
  const scale = (value) => Math.max(1, Math.round(value * factor))
  const skills = (skillMap[targetCategory] || skillMap.大数据开发).map((item) => ({ ...item, jobCount: scale(item.jobCount) }))
  const jobCount = scale(286)
  return {
    statDate: '2026-07-11',
    major: majorName,
    targetCategory,
    city: cityName,
    availableMajors,
    summary: { jobCount, entryFriendlyCount: scale(192), averageSalaryMin: 8200, averageSalaryMax: 13800, extractedSkillCount: skills.length },
    cities: [{ key: cityName || '北京', jobCount: scale(62) }, { key: '成都', jobCount: scale(48) }, { key: '上海', jobCount: scale(40) }],
    industries: [{ key: '信息技术', jobCount: scale(96) }, { key: '电子通信', jobCount: scale(54) }, { key: '智能制造', jobCount: scale(31) }],
    education: [{ key: '本科及以上', jobCount: scale(220) }, { key: '硕士及以上', jobCount: scale(41) }, { key: '不限', jobCount: scale(25) }],
    skills,
    regionalMatrix: [
      { city: '北京', category: targetCategory, jobCount: scale(62) }, { city: '北京', category: '后端开发', jobCount: scale(115) },
      { city: '成都', category: targetCategory, jobCount: scale(48) }, { city: '成都', category: '数据分析', jobCount: scale(73) },
      { city: '上海', category: targetCategory, jobCount: scale(40) }, { city: '上海', category: '人工智能', jobCount: scale(56) },
    ],
    suggestions: [
...
<template>
  <section class="university-workspace">
    ...
    <div v-if="notice" class="university-info"><CircleAlert :size="18" /><span>{{ notice }}</span></div>
